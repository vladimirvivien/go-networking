...
// net.Conn implements io.Reader which allows the code to stream data in
// until io.EOF is enocountered.
//
// Testing:
...
func handleConnection(conn net.Conn) {
	...
	for {
		// buffer for client command is made small to demonstrate
		// sreaming io strategy.
		var cmdLine []byte

		// appendBytes is a func that simulates end-of-File marker error.
		// since we are streaming IO on top of a streaming protocol, there may never be
		// an actual EOF marker.  So this function simulates it by looking for char '\n'.
		...
		// stream data until io.EOF in 4 byte chunks.
		// The chunks are kept small to demonstrate streaming.
		for {
			chunk := make([]byte, 4)
